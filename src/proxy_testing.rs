...
pub async fn proxy_test() {
	...
	// let any_addr = SocketAddr::from((Ipv4Addr::UNSPECIFIED, 0));
	// let mut client_addr: Option<SocketAddr> = None;
	// 
	// let server_sock = UdpSocket::bind(proxy_addr).await.unwrap();
	// let client_sock = UdpSocket::bind(any_addr).await.unwrap();
	// 
	// let (com_stream1, com_stream2) = tokio::io::duplex(8192);
	// let 
	// 
